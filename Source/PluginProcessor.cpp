...
//    MemoryOutputStream (destData, true).writeInt(lastUIWidth);
...
//    lastUIWidth = MemoryInputStream (data, static_cast<size_t> (sizeInBytes), false).readInt();
    // You should use this method to restore your parameters from this memory block,
    // whose contents will have been created by the getStateInformation() call.
    ScopedPointer<XmlElement> xmlState (getXmlFromBinary (data, sizeInBytes));
    if (xmlState != nullptr) {
        if (xmlState->hasTagName ("ParamTutorial")) {
            lastUIWidth = xmlState->getIntAttribute("uiWidth");
            lastUIHeight = xmlState->getIntAttribute("uiHeight");
            //filepath = xmlState->getStringAttribute ("filepath");
